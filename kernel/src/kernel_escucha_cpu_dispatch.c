...
void kernel_escucha_cpu_dispatch(){
    ...
    while (1){
		...
		log_info(kernel_logger,"Op code:%d!!!", datos_de_cpu->codigo_operacion);
		...
		switch (datos_de_cpu->codigo_operacion) //siempre debo saber el pid y el tid de quien invocó
		{
			...
			case THREAD_CREATE://genera un nuevo TCB con un TID autoincremental - mandarlo a READY
				...
				*tid_copia = nuevo_tid;
				...
				//espero la confirmacion a memoria
			...
			case THREAD_JOIN:
				...
				TCB* tcb_a_bloquear = buscar_tcbs(lista_tcbs,tid_join->TID ,tid_join->PID);//BLOQUEADO, primero se busca el TID grande para saber quien invoco al thread_join

				TCB* tcb_buscado = buscar_tcbs(lista_tcbs, tid_join->tid,tid_join->PID);//TID que invocó deepende del estado del tid chico
			...
			case THREAD_CANCEL:
				...
				TCB* tcb_a_exit = buscar_tcbs(lista_tcbs, tid_cancel->TID,tid_cancel->PID);
			...
			case THREAD_EXIT:
				...
				TCB* hilo_a_finalizar = buscar_tcbs(lista_tcbs, invocadores2->tid_inv, invocadores2->pid_inv);
			...
			case MUTEX_LOCK:	
				t_mutex_todos* recurso_lock = deserializar_mutex(datos_de_cpu);
				sem_wait(&mutex);
				PCB* pcb_buscadoRL = buscar_proceso(lista_procesos, recurso_lock->PID);
				sem_post(&mutex);
				if(pcb_buscadoRL == NULL){
					printf("No se encontro el PID: %u \n", recurso_lock->PID);
				}
				//tmb el tcb para despues ingresarlo a la lista de bloqueados
				TCB* hilo_block_mutex = buscar_tcbs(lista_tcbs, recurso_lock->TID, recurso_lock->PID);
				...
				break;
			case MUTEX_UNLOCK:
				t_mutex_todos* recurso_unlock = deserializar_mutex(datos_de_cpu);
				sem_wait(&mutex);
				PCB* pcb_buscadoRU = buscar_proceso(lista_procesos, recurso_unlock->PID);
				sem_post(&mutex);
				...
				if(unlock->tid == recurso_unlock->TID){
					...
					if (!queue_is_empty(unlock->bloqueados_mutex)){
						...
						TCB* hilo_a_ready = buscar_tcbs(lista_tcbs, tid_cola, recurso_unlock->PID);
			...
			case DUMP_MEMORY:
				...
				TCB* tcb_bloc_dump = buscar_tcbs(lista_tcbs,invocadores3->tid_inv,invocadores3->pid_inv);
			...
			case IO:
				...
				TCB* hilo_a_dormir = buscar_tcbs(lista_tcbs,tiempo_io->TID,tiempo_io->PID);
